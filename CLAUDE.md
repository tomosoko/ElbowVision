...
| `tests/test_add_patient.py` | 42 | add_patient.py（CSV_FIELDS定数・_python venv検出・append_to_csv・deduplicate_csv・build_library subprocess呼び出し） |
| `tests/test_finetune_real_xray.py` | 50 | finetune_real_xray.py（定数・_compute_bbox_from_kps・_format_yolo_label・_augment_image_and_kps・bbox+label統合・エッジケース） |
| `tests/test_inference_test.py` | 65 | inference_test.py（定数・collect_images・build_csv_row・draw_overlay・print_summary） |
| `tests/test_generate_parallel.py` | 48 | generate_parallel.py（定数・apply_domain_aug形状/dtype/値域・ヒストグラムマッチング・統計特性・CDF検証） |
| `tests/test_eval_convnext_v6.py` | 57 | eval_convnext_v6.py（定数・VAL_TRANSFORMS構造・ValDataset LAT抽出・compute_bland_altman_stats辞書構造・save_bland_altman_plot PNG出力・load_model分岐） |
| `tests/test_generate_drr_gif.py` | 41 | generate_drr_gif.py（定数・pick_volume最近傍選択・add_labelsテキスト描画・bounce往復リスト・make_gif PIL保存） |
| `tests/test_create_phantom.py` | 62 | create_phantom.py（定数・ファントム生成パラメータ・bone_material_hu・cylinder/sphere形状・volume構築・DICOMメタ設定） |
| `tests/test_test_realxray_inference.py` | 46 | test_realxray_inference.py（定数・build_drr_cdf・histogram_match・draw_keypoints・build_csv_row・print_summary） |
| `tests/test_phantom_test.py` | 68 | phantom_test.py（make_cylinder・make_ellipsoid・create_elbow_phantom・angle_between・grade・run_one・main） |
| `tests/test_generate_figures.py` | 43 | generate_figures.py（_draw_3d_box・_draw_placeholder・fig1〜fig4生成・main CLI・rcParams・エッジケース） |
| `tests/test_generate_domain_gap_figure.py` | 40 | generate_domain_gap_figure.py（edge_density・contrast_rms・hist_intersection・_load_drr_at_90・_load_dataset_drr_at_90・main CLI・エッジケース） |
| `tests/test_plot_ncc_curves.py` | 53 | plot_ncc_curves.py（snap_to_nearest_angle・compute_sharpness・compute_ncc_curve・plot_curve・main CLI・エッジケース） |
| `tests/test_generate_comparison_figure.py` | 49 | generate_comparison_figure.py（load_results・plot_per_image_bar・plot_mae_summary・plot_prediction_scatter・main CLI・エッジケース） |
| `tests/test_finetune_angle_estimator.py` | 66 | finetune_angle_estimator.py（build_model・freeze_backbone_partial・RealXrayDataset・正規化round-trip・_augment・_load_and_preprocess） |
| `tests/test_generate_comparison_figure_v6.py` | 38 | generate_comparison_figure_v6.py（LABEL_MAP・load_results・generate_fig5a_v6・generate_fig5b_v6_mae_summary・main CLI・エッジケース） |
| `tests/test_generate_training_curve_v6.py` | 28 | generate_training_curve_v6.py（load_results_csv・generate_yolo_training_curve・main CLI・エッジケース） |
