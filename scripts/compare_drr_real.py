...
# 改善済みDRR（generate_drr最適化後）
DRR_SAMPLES = {
    "AP":  os.path.join(PROJECT_ROOT, "results/domain_gap_analysis/drr_improved_AP.png"),
    "LAT": os.path.join(PROJECT_ROOT, "results/domain_gap_analysis/drr_improved_LAT.png"),
}
...
def main():
    for view in ["AP", "LAT"]:
        if not os.path.exists(REAL_XRAY[view]):
            print(f"WARNING: 実X線画像が見つかりません: {REAL_XRAY[view]}")
            sys.exit(1)
        if not os.path.exists(DRR_SAMPLES[view]):
            print(f"WARNING: DRRサンプルが見つかりません: {DRR_SAMPLES[view]}")
            sys.exit(1)
